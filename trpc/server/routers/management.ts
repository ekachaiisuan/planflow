...
const managementProcedure = protectedProcedure.use(async ({ ctx, next }) => {
  const { success } = await auth.api.userHasPermission({
    body: {
      role: (ctx.user.role ?? 'user') as Role,
        ...
        try {
          ...
          if (cause?.code === '23503') {
            throw new TRPCError({
              code: 'CONFLICT',
              message:
                'Cannot delete department because user profiles still use it',
            });
          }
        ...
        try {
          ...
          if (cause?.code === '23503') {
            throw new TRPCError({
              code: 'CONFLICT',
              message:
                'Cannot delete position because user profiles still use it',
            });
          }
        ...
        try {
          ...
          if (cause?.code === '23503') {
            throw new TRPCError({
              code: 'CONFLICT',
              message:
                'Cannot delete prefix because user profiles still use it',
            });
          }
